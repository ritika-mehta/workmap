...
import showPublishedList from "../../actions/publishedPostsAction"
import showUnderReviewList from "../../actions/UnderReviewPostsAction"
import BoostPopup from "../../components/BoostPopup";
import Contribute from "../../components/ContributePopup";
import { LinkedinShareButton, TwitterShareButton } from "react-share";
import saveBookMarkPostAction from '../../actions/BookMarkPostsAction';
import Banner from "../../components/Banner";
import Ad_Banner from "../../components/ad_banner";
import {getBannerList} from '../../actions/BannerAction';
import showBookMarkClicks from '../../actions/BookMarkClicksAction';
...
import axios from 'axios';
  ...
  const Review = (props) => {
    ...
    const publishList  = useSelector(s=>s.publishList)
    const underReviewList  = useSelector(s=>s.underReviewList)
    const [ itemnumber,setItemnumber ]  = useState(0);
    const clickamountdata  = useSelector(s=>s.boostcontent)
    const bannerlist  = useSelector(s=>s.BannerList.banner)
    const refContainer = useRef(null);
    const bookMarkClicks = useSelector(s=>s.bookMarkClicks)
    const [postType,setPostType] = useState(publishList.data && publishList.data !== false?publishList.data:[]);
    ...
    const [listType,setListType] = useState("published")


    const itemTypeHandler = (type_) =>{
      setItemtype(type_)
      console.log("whiolenoew",postType)
      console.log("type+++++",type_)
        let lists = [];
        // setPostType(allposts)
        if(type_==='all') {
          lists = publishList.data.data
        } else {

          lists = publishList.data.data.slice().filter(el =>type_===el.post_type)
        }
           setPostType(lists)
        console.log("newallpost",publishList)
    }
  ...
  useEffect(async ()=>{
    if(localStorage.getItem("user")) {
     await showPublishedList(JSON.parse(localStorage.getItem("user")).id,1)(dispatch)
     await dispatch(getallclick())
     await dispatch(getBannerList())
      console.log("newwwww",publishList);
    }
    let item = getItemInRow();
    setItemnumber(item)
  },[])
  console.log("test",publishList)

  useEffect(()=>{
    console.log("newListtttt",publishList)
    setPostType(publishList.data? publishList.data.data:[])
  },[publishList])
      ...
      const handleUnderReviewPost = async () =>{
        if(localStorage.getItem('user')) {
        }
        await showUnderReviewList(JSON.parse(localStorage.getItem("user")).id)(dispatch)
        setListType('underReview')
        console.log("underrrrrrrrrrr",(JSON.parse(localStorage.getItem("user")).id))
      }
      console.log("testinggg",underReviewList)



      const handleBookmarkPost = async (id) =>{
        // window.scrollTo(0, 0);
        ...
        if(localStorage.getItem('user') !==null) {
          ...
          console.log("postssssss",id)
        }
        else{
  
        }
        await saveBookMarkPostAction(form_Data)(dispatch)
      }
       ...
       const listUI = () => {
        ...
        let lists = [] 
        let n = 0
        console.log("list==================",publishList)
        if(postType.length > 0) {
          lists = (postType)
        }
        if(listType=="underReview") {
          if(underReviewList.data !== undefined && underReviewList.data.data !== undefined) {
          lists = underReviewList.data.data
        }
      }
        ...
        n++
          return(
            ...
            <div className="card">
              ...
              <img src={list!== undefined ? list.image: ''} alt="img" />
              <div className="card-body">
                ...
                <a href={list !== undefined ? list.url : ''} className="btn-link">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 14 14"
                ...
                <div className="profile-row d-flex flex-wrap align-items-start justify-content-between">
                  <div className="profile-wrap d-flex flex-wrap align-items-center">
                  {
                    ...
                    (imgsrc && list.userdata.name == name ) ? 
                    <img className="image-round" src={imgsrc} />
                    :
                    <span className="info-graphic-icon d-flex align-items-center justify-content-center">
                    {list !== undefined ? list.userdata.name.charAt(0) : ''}
                    </span>
                  }
                    <div className="profile-desc">
                       <span className="user-name">{list !== undefined ? list.userdata.name : ''}</span>
                      <span>{list.created_date}</span>
                    </div>
                  </div>
                  <button className="badge badge-danger">{list.category !== null ?list.category.title:''}</button>
                </div>
                <h2>
                {list !== undefined ? list.title : ''}
                </h2>
                <p>
                {WordLimit(list !== undefined ? list.description : '')}
                </p>
                <div className="card-action d-flex flex-wrap justify-content-between">
                  <button className="like-count">
                    <span className="d-flex align-items-center">
                      ...
                      {list !== undefined ? list.total_click : ''}
                    </span>
                  </button>
                  <div className="d-flex flex-wrap">
                    ...
                    {
                       list.is_bookmarked === null ?
                       <button className="btn-bookmark1" onClick = {e=>handleBookmarkPost(list.id)}>
                       
                       {/* //  list.is_bookmarked !== null ? 
                       //  <span className="hover-info color-info"> 
                       ...
                       :
                       <button className="btn-bookmark" onClick = {e=>handleBookmarkPost(list.id)}>
                       {
                       //  list.is_bookmarked !== null ? 
                       //  <span className="hover-info color-info"> 
                       //   Bookmark this article
                ...
                <div>
                  {list.user_id == JSON.parse(localStorage.getItem("user")).id ?
                  <button className="btn btn-boost" onClick={e=>modalOpenBoost(list.title)}>
                    Boost Content
                  </button>
        : ''}
                </div>
      ...
      const getTotalNoOfPgeinaton = () => {
        let totalPage = 0;
        if(publishList && publishList.data){
          totalPage = Math.ceil(publishList.data.total/publishList.data.per_page);
        }
        return totalPage;
      }
      const handlePageChange = (event, value) => {
         showPublishedList(JSON.parse(localStorage.getItem("user")).id,value)(dispatch)
      }
       ...
       <BoostPopup
        ...
        <div className="main-content">
          ...
          <section className="gallery">
            <ul className="tab-link d-flex flex-wrap">
              <li>
                <button className={`${listType==="published" ? 'active' :""}`} onClick={e=> setListType('published')}>Published</button>
              </li>
              <li>
                <button className={`${listType==="underReview" ? 'active' :""}`} onClick={e =>handleUnderReviewPost()}>Under review</button>
              </li>
            </ul>
