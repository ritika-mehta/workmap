...
import fetchAllPostsAction from '../../actions/FetchAllPotsAction';
import CategoryList from '../../actions/CategoryListAction';
...
import axios from 'axios';
    ...
    const Dashboard = (props) => {
      ...
      const allposts  = useSelector(s=>s.allPosts.posts)
      const categoryList  = useSelector(s=>s.categoryList) 
      ...
      console.log("posttyoee",postType)
      ...
      useEffect(()=>{
        ...
        if(categoryList.status==="success") {
          setButtonList(categoryList.data.data)
        }
      ...
      const itemTypeHandler = (type_) =>{
        ...
        // console.log("whiolenoew",postType)
          ...
          if(type_==='all') {
            ...
            lists = allposts.data.slice().filter(el =>type_===el.post_type)
          }
             setPostType({data:lists})
          console.log("newallpost",allposts)
      }
    ...
    const handleSeeMore = () => {
      ...
      useEffect(async ()=>{
        ...
        if(localStorage.getItem("user") || !localStorage.getItem("user")) {
          ...
          // return
          // form_Data.append("user_id",JSON.parse(localStorage.getItem("user")).id )
          await fetchAllPostsAction(form_Data,page)(dispatch)
          await CategoryList()(dispatch)
          // console.log("iddddddddd",user.token)
      ...
      console.log("categoryListtttt",categoryList)
      ...
      useEffect(()=>{
        // console.log("newListtttt",allposts)
        setPostType(allposts)
      },[allposts])
      ...
      const handlePostClickCout = async ({id,url}) =>{
        let form_Data = new FormData();
        let user={}
        if(localStorage.getItem('user')) {
          ...
          await ClickCount(form_Data)(dispatch)
      ...
      const filterPost = (btnData) =>{
        console.log("filterdata",btnData)
        console.log("filterdata2222",allposts.data)
        
        if(allposts.data.length > 0) {
          
          let lists = allposts.data.slice().filter(el => el.category == null ? true : btnData.id===el.category.id)
          setPostType({data:lists})
      ...
      const listUI = () => {
        ...
        let lists = []
        let n = 0
        if(postType !== undefined && postType.data.length > 0) {
          lists = getAllPost(postType.data)
        }
        ...
        return lists.map((list,i) => {
          ...
          return(
            ...
            <div className="card" onClick={e=> handlePostClickCout(list)} >
              ...
              <div className="card-body">
                ...
                <div className="profile-row d-flex flex-wrap align-items-start justify-content-between">
                 ...
                 list.userdata.profile_img  ? 
                <img className="image-round" src={ 'http://made.inworkmap.com/admin/public/storage/profile_image/'+ list.userdata.profile_img } />
                :           
                    ...
                    <div className="profile-desc">
                       ...
                       {/* {console.log("namelistttt====",list.userdata)} */}
                      <span>{list !== undefined ? list.created_date : ''}</span>
                    </div>
                  </div>
                 <button className="badge badge-danger">{list !== undefined ? list.category.title:''}</button>
                </div>
                <h2>
                ...
                <div className="card-action d-flex flex-wrap justify-content-between">
                  <button className="like-count">
                    ...
                    <a href={list !== undefined ? list.url : ''}>
                      <svg
                        width="25"
                        height="30"
                        viewBox="0 0 25 30"
                  ...
                  <div className="d-flex flex-wrap">
                    ...
                    {
                      list !== undefined && list.is_bookmarked === null ?
                       ...
                       <span className="d-flex align-items-center">
                         ...
                         {list !== undefined ? list.bookmark_count : ''}
                       </span>
                     </button>
                ...
                {JSON.parse(localStorage.getItem("user")) !== null && list.user_id == JSON.parse(localStorage.getItem("user")).id  ? 
                <button className="btn btn-boost" onClick={e=>modalOpenBoost(list)}>
                    Boost Content
                  </button>
                  : ''}
              </div>
      ...
      const getTotalNoOfPgeinaton = () => {
        let totalPage = 0;
        console.log("posttypesssss",postType)
        if(postType && postType.total){
          totalPage = Math.ceil(postType.total/postType.per_page);
        }
        console.log("totalPagessss",totalPage)
        return totalPage;
      ...
      const handlePageChange = (event, value) => {
        setPage(value)
        fetchAllPostsAction(value)(dispatch)
      }
      ...
      <div className="wrapper d-flex justify-content-end">
        ...
        <div className="main-content">
          ...
          {JSON.parse(localStorage.getItem("user")) == null ? 
             <button className="btn btn-boost" onClick={ handleSeeMore }>
             See more
           </button>
           : ''}
